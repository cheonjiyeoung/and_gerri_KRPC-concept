...
from __config_path__ import ROBOT_INFO, OPERATOR_INFO, VIDEO_INFO, AUDIO_INFO
import logging
...
from __commander_path__ import __commander_class__
# If the model is not predefined in SampleBaseCommander,
# we explicitly assign a sub-function implementation.

__commander_var__ = __commander_class__(channels=channels, robot_info=ROBOT_INFO, operator_info=OPERATOR_INFO)
__commander_var__.connect()

screen = app.primaryScreen()
size = screen.size()
width = size.width()
height = size.height()
window =CommonQtUI(w=width,h=height,webrtc_channels=channels,commander=__commander_var__)
