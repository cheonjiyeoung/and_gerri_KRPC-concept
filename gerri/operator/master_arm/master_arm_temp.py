...
class MasterArm:
    ...
    def connect(self):
        """
        Initialize the connection to the Dynamixel motors.
        Opens the port, sets the baudrate, _and_ enables torque for all motors.
        """
        # Open port
        if self.armDynamixel.open_port():
            print("Succeeded to open the port")
        else:
            print("Failed to open the port")
            raise Exception("Port opening failed")

        # Set port baudrate
        if self.armDynamixel.set_baud_rate(BAUDRATE):
        ...
        for _id in self.armDynamixel_id:
            comm_result, error = self.armDynamixel.enable_torque(_id, True)
            if comm_result != 0:  # Replace '0' with appropriate COMM_SUCCESS constant
                ...
                print(f"Torque enabled for Dynamixel ID {_id}")
    ...
    def readPosition(self):
        ...
        for i in range(self.n_dxls):
            if self.position_deg[i] > 360.:
                ...
                _n = int(self.position_deg[i] / 360.) - 1
                self.position_deg[i] += _n * 360.
            self.position_rad[i] = self.position_deg[i] * np.pi / 180.
...
if __name__ == '__main__':
    ...
    dxls = MasterArm(n_dxls=n_dxls, port='/dev/ttyUSB0')  # Initialize the MasterArm with the given number of joints

    try:
        dxls.connect()  # Establish connection to Dynamixel motors
        ...
        dxls.disconnect()  # Safely disconnect from the Dynamixel motors
