...
class VRController:
    ...
    def __init__(self, axis_map=None, rotation_offset_q=None, position_offset=None):
        """
        ...
        # --- 초기 좌표 변환 설정 ---
        self.set_transform(axis_map, rotation_offset_q, position_offset)
    ...
    def update(self, vr_data):
        ...
        self.left_current_pose = pin.SE3(pin.Quaternion(np.array(left_rot_xyzw)), np.array(left_pos))
        ...
        self.right_current_pose = pin.SE3(pin.Quaternion(np.array(right_rot_xyzw)), np.array(right_pos))
    ...
    def vr_delta_to_robot_delta(self, delta_pose_vr: pin.SE3) -> pin.SE3:
        """인스턴스에 저장된 변환 행렬을 사용해 델타 Pose를 변환합니다."""
        return self.T_vr_robot * delta_pose_vr * self.T_vr_robot.inverse()


    def set_transform(self, axis_map=None, rotation_offset_q=None, position_offset=None):
        """
        좌표 변환 설정을 동적으로 업데이트합니다.
        """
        print("🔄 좌표 변환 설정을 업데이트합니다.")
        self.T_vr_robot = self._build_transform(axis_map, rotation_offset_q, position_offset)


    def _build_transform(self, axis_map, rotation_offset_q, position_offset) -> pin.SE3:
