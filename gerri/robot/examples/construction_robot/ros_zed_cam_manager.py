...
class RosZedCameraManager(Node):
    """
    ...
    def __init__(self, camera_index=0, width=640, height=480, fps=30, camera_name='cam', auto_start=True):
        ...
        self.zed_stereo_frame = None
        self.bridge = CvBridge()
        self.subscription_zed_stereo = self.create_subscription(Image, '/zed/zed_node/stereo/image_rect_color', self.zed_stereo_callback, 1)
    ...
    def zed_stereo_callback(self, msg):
        # # print("Subscribing Zed")
        # print(msg.encoding, msg.height, msg.width)
        # # print(msg.)
        # print(msg.step)
        # data_bytes = bytes(msg.data)
        # frame = np.frombuffer(data_bytes, dtype=np.uint8).reshape((msg.height, msg.width, 4))  # RGBA
        # rgb = frame[:, :, :3]  # Drop alpha channel
        # self.zed_stereo_frame = rgb

        zed_frame = self.bridge.imgmsg_to_cv2(msg)
        self.zed_stereo_frame = cv2.cvtColor(zed_frame, cv2.COLOR_BGR2RGB)


    def get_ros_frames(self):
        if self.zed_stereo_frame is None:
            # print("No Zed Frame")
            return None
        else:
            frame = self.zed_stereo_frame
