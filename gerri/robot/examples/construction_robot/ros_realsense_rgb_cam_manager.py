...
class RosRealsenseRGBCameraManager(Node):
    """
    ...
    def __init__(self, camera_index=0, width=640, height=480, fps=30, camera_name='cam', auto_start=True):
        ## For ROS ######
        super().__init__('realsense_rgb_sub')
        self.bridge = CvBridge()
        self.subscription_rgb_left = self.create_subscription(Image, '/camera1/camera1/color/image_raw', self.rgb_left_callback, 5)
        self.subscription_rgb_right = self.create_subscription(Image, '/camera2/camera2/color/image_raw', self.rgb_right_callback, 5)
    ...
    def rgb_left_callback(self, msg):
        left_frame = self.bridge.imgmsg_to_cv2(msg)
        self.image_left_frame = cv2.cvtColor(left_frame, cv2.COLOR_RGB2BGR)

    def rgb_right_callback(self, msg):
        right_frame = self.bridge.imgmsg_to_cv2(msg)
        self.image_right_frame = cv2.cvtColor(right_frame, cv2.COLOR_RGB2BGR)
