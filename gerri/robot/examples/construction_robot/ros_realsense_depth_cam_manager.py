...
class RosRealsenseDepthCameraManager(Node):
    """
    ...
    def __init__(self, camera_index=0, width=640, height=480, fps=30, camera_name='cam', auto_start=True):
        ## For ROS ######
        super().__init__('realsense_depth_sub')
        self.bridge = CvBridge()
        self.subscription_depth_left = self.create_subscription(Image, '/camera1/camera1/aligned_depth_to_color/image_raw', self.depth_left_callback, 5)
        self.subscription_depth_right = self.create_subscription(Image, '/camera2/camera2/aligned_depth_to_color/image_raw', self.depth_right_callback, 5)
        # self.subscription_depth = self.create_subscription(Image, '/camera/camera/depth/image_rect_raw', self.depth_callback, 10)
        # self.subscription_depth = self.create_subscription(Image, '/camera/camera/aligned_depth_to_color/image_raw',
        #                                                    self.depth_callback, 10)
        self.image_left_frame = None
        self.image_right_frame = None
    ...
    def depth_left_callback(self, msg):
        ...
        depth_8bit = cv2.convertScaleAbs(depth_clipped, alpha=(255.0 / 8000.0), beta=30)
        depth_colored = cv2.applyColorMap(depth_8bit, cv2.COLORMAP_BONE)
        self.image_left_frame = depth_colored
    ...
    def depth_right_callback(self, msg):
        ...
        depth_8bit = cv2.convertScaleAbs(depth_clipped, alpha=(255.0 / 8000.0), beta=30)
        depth_colored = cv2.applyColorMap(depth_8bit, cv2.COLORMAP_BONE)
        self.image_right_frame = depth_colored


    def get_ros_frames(self):
        if self.image_left_frame is None and self.image_right_frame is None:
            return None
        else:
            frame = np.hstack((self.image_left_frame, self.image_right_frame))
            # print("Total frame:", frame.shape)
            return frame
    #### FOR ROS2 ######
