...
class GydSubController:
    ...
    def _update_loop(self):
        ...
        while True:
            ...
            if pose_x > 36:
                self.status.current_floor = 3
            if pose_x < 36:
                self.status.current_floor = 2
    ...
    def move_waypoint_async(self,value=None,option=None):
        poi = value.get("poi",None)
        solar_callback = option.get("solar_callback",False)
        uuid = option.get("uuid",None)
        self.robot_controller.move_waypoint_async(poi=poi,solar_callback=solar_callback,uuid=uuid)
